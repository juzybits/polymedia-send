// Forked from `sui/sdk/zksend/src/links/builder.ts`.
...
import { MAINNET_CONTRACT_IDS, ZkBag, ZkBagContractOptions } from '@mysten/zksend';
...
export class ZkSendLinkBuilder {
	...
	#createMultiSendTransaction(
		...
		const address = this.keypair.toSuiAddress();
		const objectsToTransfer = [...this.objectIds].map((id) => txb.object(id));
		const [gas] = txb.splitCoins(txb.gas, [roundedGasAmount]);
		objectsToTransfer.push(gas);
